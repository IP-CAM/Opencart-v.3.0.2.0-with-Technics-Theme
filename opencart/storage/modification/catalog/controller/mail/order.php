...
class ControllerMailOrder extends Controller {
	...
	public function add($order_info, $order_status_id, $comment, $notify) {
		...
		//technics	
		if (is_file(DIR_IMAGE . $this->config->get('theme_technics_mail_logo'))) {
			$data['logo'] = $order_info['store_url'] . 'image/' . $this->config->get('theme_technics_mail_logo');
		} else {
			$data['logo'] = '';
		}
		$data['text_technics_mail'] = $language->get('text_technics_mail');
		$data['header_text_logo'] = html_entity_decode($this->config->get('theme_technics_header_text_logo'), ENT_QUOTES, 'UTF-8');
		$data['technics_mail_text'] = html_entity_decode($this->config->get('theme_technics_mail_text' . $this->config->get('config_language_id')), ENT_QUOTES, 'UTF-8');
		$data['technics_mail_about'] = html_entity_decode($this->config->get('theme_technics_mail_about' . $this->config->get('config_language_id')), ENT_QUOTES, 'UTF-8');
		$data['theme_color'] = $this->config->get('theme_technics_color');
		if ($this->config->get('theme_technics_color')) {
			if ($this->config->get('theme_technics_color') == 'custom') {
				...
				$colors = explode(' ', $this->config->get('theme_technics_color'));
		...
		$data['theme_technics_checkout_st3'] = $this->config->get('theme_technics_checkout_st3_sa');
...
// technics
				...
				if($this->config->get('theme_technics_social_nav')){
				$data['social_navs'] = $this->config->get('theme_technics_social_nav');
				}				
				foreach ($data['social_navs'] as $key => $social_nav) {
					$data['social_navs'][$key]['icone'] = $order_info['store_url'] . 'catalog/view/theme/technics/images/icons/mail-png/social_'.$social_nav['settype'].'.png';
				}
// technics end
...
// technics
					...
					if ($this->config->get('theme_technics_image_cart_resize')) {
						if ($product_info['image']) {
							$image = $this->model_tool_image->technics_resize($product_info['image'],  $this->config->get('theme_technics_image_cart_width'), $this->config->get('theme_technics_image_cart_height'));
						} else {
							$image = $this->model_tool_image->technics_resize('placeholder.png',  $this->config->get('theme_technics_image_cart_width'), $this->config->get('theme_technics_image_cart_height'));
						}
					} else {
						if ($product_info['image']) {
							$image = $this->model_tool_image->resize($product_info['image'],  $this->config->get('theme_technics_image_cart_width'), $this->config->get('theme_technics_image_cart_height'));
						} else {
							$image = $this->model_tool_image->resize('placeholder.png',  $this->config->get('theme_technics_image_cart_width'), $this->config->get('theme_technics_image_cart_height'));
						}
					}
// technics end
			...
			$data['products'][] = array(
				...
				'href'     => $this->url->link('product/product', 'product_id=' . $order_product['product_id']),// technics
		...
		$mail->setTo($order_info['email']);
				...
				//technics start
				if(!$order_info['email']){
					$mail->setTo('nomailtechnicss@nomail.ru');
				}
				//technics stop
