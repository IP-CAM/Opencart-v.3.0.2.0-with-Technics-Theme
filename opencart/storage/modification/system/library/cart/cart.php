...
class Cart {
	...
	public function getProducts() {
		...
		foreach ($cart_query->rows as $cart) {
            ...
            if ($product_option_id !== 'suboptions') {
					...
					$option_query = $this->db->query("SELECT po.product_option_id, po.option_id, od.name, o.type FROM " . DB_PREFIX . "product_option po LEFT JOIN `" . DB_PREFIX . "option` o ON (po.option_id = o.option_id) LEFT JOIN " . DB_PREFIX . "option_description od ON (o.option_id = od.option_id) WHERE po.product_option_id = '" . (int)$product_option_id . "' AND po.product_id = '" . (int)$cart['product_id'] . "' AND od.language_id = '" . (int)$this->config->get('config_language_id') . "'");
				        ...
				        foreach ($value as $prod_opt_id => $prod) {
				            foreach ($prod as $option_value_id => $suboption) {
				                ...
				                $suboptionExp[$prod_opt_id][$option_value_id]['suboption_name'] = $tmp[0];
				                $suboptionExp[$prod_opt_id][$option_value_id]['suboption_price'] = $tmp[1];
                            }
                        }
                    }
			

					if ($option_query->num_rows || $suboptionExp) {
						...
						} elseif ($option_query->row['type'] == 'checkbox' && is_array($value) || is_object($value)) {
							foreach ($value as $product_option_value_id) {
                if (!is_object($product_option_value_id)) {
								$option_value_query = $this->db->query("SELECT pov.option_value_id, pov.quantity, pov.subtract, pov.price, pov.price_prefix, pov.points, pov.points_prefix, pov.weight, pov.weight_prefix, ovd.name FROM " . DB_PREFIX . "product_option_value pov LEFT JOIN " . DB_PREFIX . "option_value_description ovd ON (pov.option_value_id = ovd.option_value_id) WHERE pov.product_option_value_id = '" . (int)$product_option_value_id . "' AND pov.product_option_id = '" . (int)$product_option_id . "' AND ovd.language_id = '" . (int)$this->config->get('config_language_id') . "'");
                 }
                                if (is_object($product_option_value_id)) {
								    $option_value_query->row['suboptions'] = $suboptionExp;
                                    $option_value_query->row['price_prefix'] = '+';
                                }

								if ($option_value_query->num_rows || is_object($product_option_value_id)) {
									if ($option_value_query->row['price_prefix'] == '+') {
										$option_price += $option_value_query->row['price'];
                            ...
                            foreach ($value as $product_option_value_id) {
                                ...
                                if ($option_value_query->num_rows || (is_object($value) && !empty($value))) {
                                    if ($option_value_query->row['price_prefix'] == '+') {
                                        $option_price += $option_value_query->row['price'];
                                        foreach ($option_value_query->row['suboptions'] as $prod_opt_id => $prod) {
                                            foreach ($prod as $suboption_props) {
...
// technics start
			if ($this->config->get('theme_technics_product_cod')) {
				$product_query->row['model'] = $product_query->row[$this->config->get('theme_technics_product_cod')];
			}
// technics end
