...
class ControllerExtensionModuleTechnicsMainSlider extends Controller {
	public function index($setting) {
		...
		$data['slide_width'] = 'slides--width-' . $setting['slide_width'];
			...
			foreach ($setting['slider_image'] as  $result) {
				...
				if (is_file(DIR_IMAGE . $result['image'])) { 
					$data['banners'][$result['sort_order']][] = array(
						...
						'image' => isset($setting['resize']) ? $this->model_tool_image->technics_resize($result['image'], $setting['width'], $setting['height']) : $this->model_tool_image->resize($result['image'], $setting['width'], $setting['height'])
					);
				}
			}			
		}
